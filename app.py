...
def load_data():
    ...
    # Force 1000 if file small
    if len(df) < 1000:
        df = pd.concat([df]*20, ignore_index=True).head(1000)
...
col3.metric("Avg CSAT", f"{df['CSAT_Score'].mean():.2f}/5")
col4.metric("Avg MTTR", f"{df['Resolution_Time_Hours'].mean():.1f} hrs")
col5.metric("Repeat Rate", f"{df['Is_Repeat_Ticket'].mean()*100:.1f}%")
...
st.subheader("Raw Data Preview - 1000 rows")
